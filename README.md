...
Python 3: Stellen Sie sicher, dass Python 3 auf Ihrem System installiert ist. Sie können Python von https://python.org herunterladen.
...
CSV-Datei Monthly Payments von Etsy herunterladen: Laden Sie die gewünschte CSV-Datei aus Ihrem Etsy-Shop herunter, diese können Sie in input.csv umbenennen.

CSV-Datei Orders von Etsy herunterladen: Laden Sie aus Shop Manager > Einstellungen > Optionen die Orders für den Monat herunter und ändern Sie den Namen der CSV-Datei in orders.csv um.

Skript herunterladen: Laden Sie die Datei etsy_to_lexoffice.py herunter und speichern Sie sie im gleichen Verzeichnis wie die input.csv und orders.csv

Skript ausführen: Öffnen Sie Ihre Konsole, navigieren Sie zum Verzeichnis der Dateien und führen Sie das Python-Skript aus.
...
CSV-Datei in Lexoffice importieren: Importieren Sie die Datei output.csv in Lexoffice.
...
Read the data from input.csv (Etsy monthly statement CSV).

Read the data from orders.csv (Etsy orders CSV).

Process the data, including handling sales, deposits, fees, and refunds (both full and partial).

Create a new, formatted CSV file named output.csv suitable for import into Lexoffice.

Generate a log file named convert_csv_[Date]_[Time].log with detailed information about the conversion process.
